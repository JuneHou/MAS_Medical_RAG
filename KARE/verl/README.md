...
2. Runs full 3-round debate for each patient:
   - Round 1: Target patient analysis (with retrieval)
   - Round 2: Mortality risk + protective factor analysis (parallel, with retrieval)
   - Round 3: Integrator assessment (with mortality + survival specific retrieval)
3. Extracts debate history (rounds 1-3 only, before integrator runs)
4. Constructs two training prompts per patient:
   - **Mortality prompt**: Target patient + debate history + mortality retrieved docs
   - **Survival prompt**: Target patient + debate history + survival retrieved docs
5. Saves to Parquet with ground truth labels
...
2. **Target Patient Context**:
   - Demographics, diagnoses, procedures, medications
   - Lab values, vital signs
   - Previous admission history

3. **Debate History** (rounds 1-3):
   - Round 1: Target patient analysis with initial risk assessment
   - Round 2: Mortality risk factors (from similar mortality cases)
   - Round 3: Protective factors (from similar survival cases)

4. **Retrieved Medical Evidence** (assessment-specific):
   - Mortality prompt: Documents retrieved for mortality risk query
   - Survival prompt: Documents retrieved for survival factors query
   - Format: `[Document N] Title\nContent...`

5. **Task Instruction**:
   ```
   Now provide your [mortality/survival] probability assessment with comprehensive reasoning.
   ```
