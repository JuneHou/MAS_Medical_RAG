...
def run_single_agent_evaluation(start_idx: int = 0,
    ...
    """
    Run KARE single-agent evaluation using effGen.
    ...
    """
    print(f"Initializing KARE Single-Agent {mode.upper()} Evaluation (effGen)...")
    ...
    try:
        ...
        if mode.lower() == "rag":
            agent_system = MortilitySingleAgentEffGenRAG(
            ...
            agent_system = MortilitySingleAgentEffGenCoT(
                model_name=model_name,
    ...
    try:
            ...
            for i in pbar:
                try:
                    ...
                    if output_path and (len(results) % batch_size == 0):
                        ...
                        save_results(results, metrics, output_path, mode, in_context)
                        print(f"\nIntermediate save: {len(results)} results, Accuracy: {metrics.get('accuracy', 0):.3f}")
                    ...
                    if error_count > 50:
                        print(f"WARNING: Too many errors ({error_count}), stopping.")
                        break
        ...
        if output_path and results:
            ...
            print(f"Fallback Rate: {metrics.get('fallback_predictions', 0)}/{metrics.get('total_samples', 0)}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="KARE Single-Agent Mortality Prediction (effGen)")
    ...
    if args.output is None:
        ...
        if args.mode == 'rag':
            dir_name = f"single_effgen_{args.mode}_{clean_model_name}_{args.retriever_name}_{in_context_suffix}"
        else:
            dir_name = f"single_effgen_{args.mode}_{clean_model_name}_{in_context_suffix}"
