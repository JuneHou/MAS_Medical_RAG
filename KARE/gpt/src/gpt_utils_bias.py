...
def retrieve_documents(medrag, query: str, k: int = 8) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for i, (snippet, score) in enumerate(zip(retrieved_snippets, scores)):
            # Handle both string snippets and dict snippets
...
def run_gpt_integrator_no_search(
    gpt_client,
    target_context: str,
    analyst1_output: str,
    analyst2_output: str,
    ...
    """
    Run GPT integrator WITHOUT search capability (for Conditions B/C/D).
    
    This is used when retrieval is predetermined (reused from other conditions)
    rather than dynamically requested by the integrator.
    ...
    Args:
        ...
        retrieved_docs: Optional retrieved documents (formatted string)
        
    Returns:
        Integrator response with prediction
    """
    # Get system prompt WITHOUT search tool
...
"""
    ...
    # Build prompt with optional retrieval
    if retrieved_docs:
