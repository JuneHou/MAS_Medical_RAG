#!/usr/bin/env python3
"""
Condition C' — Qwen analysts + oss-120b retrieval + oss-120b integrator.

Mirrors KARE/gpt/src/run_condition_C.py byte-for-byte EXCEPT the LLM client:
swaps GPTClient -> ARCClient. The analyst outputs are loaded from the existing
Qwen RAG debate logs; the retrieval bundle (query + docs) is reused from
Cond A' (oss-120b retrieval). The integrator is oss-120b via ARC, using the
`balanced_clinical_integrator_no_search` prompt — which already carries the
bias text and does NOT require a forced-search patch (docs are pre-supplied).
...
Usage:
    ...
    python DAIH/experiments/run_condition_C_oss.py
"""
...
from run_condition_C import process_sample_condition_c  # noqa: E402
...
DEFAULT_OUTPUT = REPO_ROOT / "DAIH" / "results" / "condition_C_oss"


def main():
    p = argparse.ArgumentParser(description="Condition C' — Qwen analysts + oss retrieval + oss integrator")
    ...
    p.add_argument("--condition_a_dir", type=str, default=str(DEFAULT_COND_A_DIR),
                   help="Directory containing Cond A' logs (retrieval bundle comes from here)")
    ...
    print("=" * 80)
    print(f"CONDITION C' — Qwen analysts + oss retrieval (cached) + oss integrator")
    ...
    summary = []
    for sample in tqdm(pending, desc="cond_C_oss"):
        sid = sample["sample_id"]
        try:
            result = process_sample_condition_c(sample, arc_client, cond_a_dir, str(qwen_log_dir))
